...
Most of this recovers by itself: Firefox and Chromium can both resolve a profile from the command line, so when Finicky cannot read the list it hands the profile over unresolved and lets the browser do the lookup it was always going to do. A classic `profiles.ini` profile keeps working by name under a denial. A profile created with Firefox's newer profile manager does not, because only the profile group store maps its name to a directory, and that is the file Finicky cannot read. For those, and to get the profile list in the Finicky window back, there are two ways out, and macOS will not prompt you for either one:
